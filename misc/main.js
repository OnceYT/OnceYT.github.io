...
document.addEventListener("DOMContentLoaded", function() {
  ...
  setTimeout(function() {
    document.querySelectorAll('.background, #hbd, .text p, .main-nav').forEach(function(element) {
...
window.addEventListener('touchmove', (e) => {
  ...
  adjustLastMousePosition(touchPosition);

  const now = new Date().getTime();
  const hasMovedFarEnough = calcDistance(last.starPosition, touchPosition) >= config.minimumDistanceBetweenStars;
  const hasBeenLongEnough = calcElapsedTime(last.starTimestamp, now) > config.minimumTimeBetweenStars;

  if (hasMovedFarEnough || hasBeenLongEnough) {
    createStar(touchPosition);
    updateLastStar(touchPosition);
  }
